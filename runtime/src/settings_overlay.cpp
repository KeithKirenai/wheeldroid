...
// Frame timing measurement for CPU and GPU metrics
using Clock = std::chrono::steady_clock;
static Clock::time_point s_frameStartTime{};
static Clock::time_point s_lastPresentEndTime{};
static float s_cpuTimeMs = 0.0f;
static float s_gpuTimeMs = 0.0f;
static std::atomic<bool> s_hasFrameStart{false};
...
void RecordFrameStart() noexcept {
    s_frameStartTime = std::chrono::steady_clock::now();
    s_hasFrameStart.store(true, std::memory_order_relaxed);
}

void RecordFrameEnd() noexcept {
    const auto now = std::chrono::steady_clock::now();
    if (s_hasFrameStart.load(std::memory_order_relaxed)) {
